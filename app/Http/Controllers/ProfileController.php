...
class ProfileController extends Controller
{
    ...
    {
        $messages = Message::userMessages(Auth::user()->id)->paginate(10);
    ...
    public function sendMessageToAdmin(Request $request)
    {
       $data =  $request->validate([
        ...
        if(Setting::where('name', 'site_get_sms_new_message')->first()->value == 1){
            sms::send(Setting::where('name', 'site_mobile_number')->first()->value , "سلام \n شما یک پیام جدید با عنوان ". $request->title ." دارید \n لطفا به سایت مراجعه کنید \n " . Setting::where('name', 'site_name')->first()->value);
        }

                ...
                <form class="form-horizontal" action="{{ route('admin.discount.update' , $discount->id) }}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="checkbox" {{$discount->number_discount_fixed == -1 ? 'checked' : ''}} onclick="document.getElementById('number_discount_fixed').toggleAttribute('disabled');document.getElementById('number_discount_fixed').setAttribute('value' , null);" id="withoutLimitNumberDiscount" placeholder="بدون محدودیت">
                            <input type="number" {{$discount->number_discount_fixed == -1 ? 'disabled' : ''}} name="number_discount_fixed" id="number_discount_fixed"  class="form-control" placeholder="{{ $discount->number_discount_fixed == -1 ? 'بدون محدودیت' : $discount->number_discount_fixed}}" value="{{ $discount->number_discount_fixed == -1 ? null : $discount->number_discount_fixed}}">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="checkbox" {{$discount->max_discount_price == 0 ? 'checked' : ''}} onclick="document.getElementById('max_discount_price').toggleAttribute('disabled');document.getElementById('max_discount_price').setAttribute('value' , null);" id="unlimitedDiscountPrice" placeholder="بدون محدودیت">
                            <input type="number" {{$discount->max_discount_price == 0 ? 'disabled' : ''}} name="max_discount_price" id="max_discount_price" class="form-control" placeholder="{{ $discount->max_discount_price == 0 ? 'بدون محدودیت' : ''}}" value="{{ $discount->max_discount_price == 0 ? null : $discount->max_discount_price}}">
                        </div>
